...
def create_model_dna(args):
	...
	model = tf.keras.models.Sequential([
		...
		tf.keras.layers.Conv1D(filters=147-(2*args.trim), kernel_size=5, padding='same', activation='relu', kernel_regularizer=kreg, name="conv1" + reg ),
		tf.keras.layers.Conv1D(filters=147-(2*args.trim), kernel_size=5, padding='same', activation='relu', kernel_regularizer=kreg, name="conv2" + reg ),
		tf.keras.layers.Conv1D(filters=147-(2*args.trim), kernel_size=5, padding='same', activation='relu', kernel_regularizer=kreg, name="conv3" + reg ),
		tf.keras.layers.Flatten(),
...
def create_model_blend(args):
	...
	model_ = tf.keras.layers.Conv1D(filters=147-(2*args.trim), kernel_size=3, padding='same', activation='relu' )(model_)
	model_ = tf.keras.layers.Conv1D(filters=147-(2*args.trim), kernel_size=3, padding='same', activation='relu' )(model_)
	model_ = tf.keras.layers.Conv1D(filters=147-(2*args.trim), kernel_size=3, padding='same', activation='relu' )(model_)
	model_ = tf.keras.layers.Flatten()(model_)

	other_ = tf.keras.layers.Input(shape=(1,), dtype=tf.string)
